...
export function getIdArray(data: { id: string }[]): string[] {
	return data.map((record) => {
		return record.id;
	});
}
// перенвдит массив записей в массив их id
export function getIdArrayAutor(data: { id_author: string }[]): string[] {
	return data.map((record) => {
		return record.id_author;
	});
}
// перенвдит массив записей в массив их id
export function getIdArrayCategory(data: { id_category: string }[]): string[] {
	return data.map((record) => {
