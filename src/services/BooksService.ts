...
import { BookRepository, RowRepository } from "../repositories/_index";
...
export class BooksService {
	...
	public async getBookById(id: number) {
		const resultBookRecord = await this.rowRepository.getRow("book", id);
		if (!resultBookRecord.success) {
	...
	public async getBooks(
		...
		let raitingRecords: RatingRecord[] = <RatingRecord[]>resultRaiting.result;
		...
		for (let index = 0; index < bookRecords.length; index++) {
			...
			const resultCurrencyRecords = await this.bookRepository.getAllFromTable(
				"currency",
				{ field: "id", values: [bookRecord.currency] }
			);
			if (!resultCurrencyRecords.success)
				return { success: false, result: <Book[]>[] };
			let currencyrecord = <CurrencyRecord[]>resultCurrencyRecords.result;
			...
			books.push({
				...
				currency: { id: currencyrecord[0].id, name: currencyrecord[0].name },
	...
	public async editBook(
		...
		// создаем валюту
		let resultCurrency = await this.createCurrency(bookData.currency);
		if (!resultCurrency.success) return { success: false, result: <Book>{} };
		...
		let resultbook = await this.rowRepository.updateRow("book", {
			id: bookId,
			name: bookData.name,
			language: bookData.language,
			price: bookData.price,
			currency: resultCurrency.result?.id,
		...
		const resultBookAuthor = await this.createBookAuthor(
			bookId,
		...
		const bookDetails = {
			...
			categories: resultCategory.result,
			authors: resultAuthor.result,
		};
		const book = { ...resultbook.result, ...bookDetails } as Book;

		// перезапись рейтинга
		const resultRaiting = await this.createRaiting({
			id: uuidv4(),
			user: bookData.user,
			book: book,
		...
		if (resultCommonRaiting.success) {
			let values = 0;
			let raitingRecords: RatingRecord[] = <RatingRecord[]>(
	...
	// export interface IBookPayload { id,name,categories,language,price,currency,published,authors,raiting,user,}
	...
	public async createBooks(bookData: IBookPayload[]): Promise<{
		...
		let result: { success: boolean; bookId: string; book: Book }[] = [];
		...
		for (let index = 0; index < bookData.length; index++) {
			...
			let resultCurrency = await this.createCurrency(bookPay.currency);
			if (!resultCurrency.success) {
	...
	public async createCurrency(
		...
		return await this.bookRepository
			...
			.then(
				(data) => {
					if (data.success) {
						let currencyRecord: CurrencyRecord = <CurrencyRecord>data.result;
	...
	public async createRaiting(
		...
		let resultUser = await this.userRepository.getUser(raitingData.user);
	...
	public async DeleteBookById(id: number) {
		const res = await this.rowRepository.deleteRow("book", id);
